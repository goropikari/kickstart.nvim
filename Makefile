.PHONY: install-lua-language-server install-hooks
...
export PATH := $(LUA_LANGUAGE_SERVER_DIR)/bin:$(PATH)
...
.PHONY: lint-lua
lint-lua:
	lua-language-server --check . --checklevel=Warning --configpath=$(CURDIR)/.luarc.json
...
install-lua-language-server:
	...
	@if [ ! -x "$(LUA_LANGUAGE_SERVER_DIR)/bin/lua-language-server" ]; then \
		...
		if [ -e "$(LUA_LANGUAGE_SERVER_DIR)" ]; then \
			backup=$$(mktemp -d "$(TOOLS_DIR)/lua-language-server-old.XXXXXX"); \
			rmdir "$$backup"; \
	...
	@test -x "$(LUA_LANGUAGE_SERVER_DIR)/bin/lua-language-server" || (echo 'failed to install lua-language-server' >&2; exit 1)
